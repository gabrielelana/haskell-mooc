...
countdownPrint n = print n >> countdownPrint (n - 1)
...
whenM :: IO Bool -> IO () -> IO ()
whenM cond op = cond >>= flip when op
...
debug s op = putStrLn s *> op <* putStrLn s

-- [ALTERNATIVE IMPLEMENTATION] with do notation
-- debug :: String -> IO a -> IO a
-- debug s op = putStrLn s >> op >>= \a -> putStrLn s >> return a
