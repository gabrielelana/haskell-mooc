...
buildList _ count end | count <= 0 = [end]
...
sums n = go 1 n 0
  where go i n _ | i > n = []
        go i n a = let e = i + a in e : go (i + 1) n e
...
indexDefault (a : _) 0 _ = a
indexDefault (_ : as) i def = indexDefault as (i - 1) def
...
sorted [] = True
sorted [_] = True
sorted (n : m : rest) = n <= m && sorted (m : rest)
...
sumsOf xs = go xs 0
  where go [] _ = []
        go (x : xs) a = let e = x + a in e : go xs e
...
mymaximum f biggest [] = biggest
mymaximum f biggest (a : as) = mymaximum f (if f a biggest then a else biggest) as
