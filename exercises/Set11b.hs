...
appendAll r xs = modifyIORef r (\s -> s ++ mconcat xs)
...
hFetchLines h = do
  isEOF <- hIsEOF h
  if isEOF then return [] else (:) <$> hGetLine h <*> hFetchLines h

-- [ALTERNATIVE IMPLEMENTATION]
-- hFetchLines :: Handle -> IO [String]
-- hFetchLines h = hIsEOF h >>= thenOr (return []) ((:) <$> hGetLine h <*> hFetchLines h)
--   where thenOr :: a -> a -> Bool -> a
--         thenOr at af b = if b then at else af

-- [ALTERNATIVE IMPLEMENTATION] if we don't care about memory allocation / file dimension
-- hFetchLines :: Handle -> IO [String]
...
hSelectLines h wantedIndexes = go h (sort wantedIndexes) 1
  where go :: Handle -> [Int] -> Int -> IO [String]
        go h [] _ = return []
        go h wantedIndexes@(wantedIndex : rest) lineIndex = do
          line <- hGetLine h
          if wantedIndex == lineIndex
            then (line :) <$> go h rest (lineIndex + 1)
            else go h wantedIndexes (lineIndex + 1)
...
interact' f state = do
  line <- getLine
  let (continue, message, state') = f (line, state)
  putStrLn message
