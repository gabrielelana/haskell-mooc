...
get :: UpDown -> Int
get (Up counter) = counter
get (Down counter) = counter

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (Up counter) = Up (counter + 1)
tick (Down counter) = Down (counter - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Up counter) = Down counter
toggle (Down counter) = Up counter
...
fromList [] = Empty
fromList ((k, v) : tail) = Pair k v $ fromList tail
...
toNat :: Int -> Maybe Nat
toNat n | n < 0 = Nothing
        | otherwise = Just $ safeToNat n
        where safeToNat 0 = Zero
              safeToNat n = PlusOne $ safeToNat (n - 1)
