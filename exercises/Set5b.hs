...
treeMax Empty = 0
treeMax (Node n l r) = max n (max (treeMax l) (treeMax r))
...
allValues p (Node a l r) = p a && allValues p l && allValues p r
...
cull value (Node a l r) = if value == a then Empty else Node a (cull value l) (cull value r)
...
walk (StepL : next) (Node _ l _) = walk next l
walk (StepR : next) (Node _ _ r) = walk next r
...
set (StepL : next) val (Node a l r) = Node a (set next val l) r
set (StepR : next) val (Node a l r) = Node a l (set next val r)
...
search val tree = go val tree []
  where go :: Eq a => a -> Tree a -> [Step] -> Maybe [Step]
        go _ Empty _ = Nothing
        go val (Node a l r) path
          | val == a = Just $ reverse path
          | otherwise = case go val l (StepL : path) of
                          (Just path) -> Just path
                          Nothing -> go val r (StepR : path)
