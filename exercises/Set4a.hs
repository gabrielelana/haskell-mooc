...
allEqual [] = True
allEqual [_] = True
allEqual (a1 : a2 : as) = a1 == a2 && allEqual (a2 : as)
...
distinct [] = True
distinct (a : as) = notElem a as && distinct as

------------------------------------------------------------------------------
-- Ex 3: implement the function middle that returns the middle value (not the
-- smallest or the largest) out of its three arguments.
--
...
middle x y z = let [_, b, _] = sort [x, y, z] in b
...
incrementKey k [] = []
incrementKey k ((k', v') : kvs) = (k', v' + (if k == k' then 1 else 0)) : incrementKey k kvs

-- [ALTERNATIVE IMPLEMENTATION]
-- incrementKey k = map (f k)
--   where f :: (Eq k, Num v) => k -> (k, v) -> (k, v)
--         f k (k', v') = (k', v' + (if k == k' then 1 else 0))
...
freqs = foldr (Map.alter insertElem) Map.empty
  where
    insertElem :: Maybe Int -> Maybe Int
    insertElem Nothing = Just 1
    insertElem (Just n) = Just $ n + 1
  ...
  | otherwise =
      case (Map.lookup from bank, Map.lookup to bank) of
        (Just fromBalance, Just toBalance)
          | fromBalance >= amount -> Map.adjust (+ (- amount)) from $ Map.adjust (+ amount) to bank
          | otherwise -> bank
        (_, _) -> bank
...
maxIndex = fst . maximumBy (comparing snd) . Array.assocs
