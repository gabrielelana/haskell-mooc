...
-- Create a picture by "drawing" a picture (limited by a shape) over a base picture
over :: Picture -> Shape -> Picture -> Picture
over (Picture f1) shape (Picture f2) = Picture f
  where f :: Coord -> Color
        f c@(Coord x y) = if contains shape x y then f1 c else f2 c

paintSolid :: Color -> Shape -> Picture -> Picture
paintSolid color shape = over (fill color shape) shape
...
instance Transform Fill where
  apply (Fill c) (Picture _) = Picture (const c)
...
blur :: [Color] -> Color
blur cs =
  let (Color r g b) = foldl add black cs
      l = length cs
  in Color (r `div` l) (g `div` l) (b `div` l)

instance Transform Blur where
  apply _ (Picture f) = Picture b
    where b :: Coord -> Color
          b c@(Coord x y) = blur (map f [c, Coord (x - 1) y, Coord (x + 1) y, Coord x (y + 1), Coord x (y - 1)])
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  apply (BlurMany 0) = apply Blur
  apply (BlurMany 1) = apply Blur
  apply (BlurMany n)
    | n > 1 = apply $ Chain Blur $ BlurMany (n - 1)
    | otherwise = error "Cannot apply Blur for a negative number of times"
