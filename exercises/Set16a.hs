...
isSorted xs = property $ xs == sort xs

-- [ALTERNATIVE IMPLEMENTATION]
-- isSorted :: (Show a, Ord a) => [a] -> Property
-- isSorted [] = property True
-- isSorted [x] = property True
-- isSorted (x : y : tail) = property (x <= y) .&&. isSorted (y : tail)
...
-- [ALTERNATIVE IMPLEMENTATION]
-- inputInOutput :: (Show a, Eq a) => [a] -> [(a,Int)] -> Property
-- inputInOutput input output = forAll (elements input) (\a -> isJust $ lookup a output)
...
outputInInput input output = forAll (elements output) (\(x, n) -> property $ length (filter (== x) input) === n)
...
frequenciesProp freq input =
  let input' = getNonEmpty input
      output = freq input'
  in sumIsLength input' output .&&. inputInOutput input' output .&&. outputInInput input' output
...
genList :: Gen [Int]
genList = chooseInt (3, 5) >>= (\len -> sort <$> vectorOf len (chooseInt (0, 10)))
...
instance Arbitrary Arg where
  arbitrary = oneof [ Number <$> chooseInt (0, 10)
